 ...
 *     True on success, false if out of memory.
 */
static bool handle_jumped_spills(CodeBuffer *code, const HostX86Context *ctx,
                                 int branch_insn)
{
    RTLUnit * const unit = ctx->unit;

    const int target_block =
        unit->label_blockmap[unit->insns[branch_insn].label];
         ...
         insn_index++)
    {
        ...
        switch ((RTLOpcode)insn->opcode) {
          ...
          case RTLOP_GOTO:
            ...
            if (!handle_jumped_spills(&code, ctx, insn_index)) {
                return false;
            }
