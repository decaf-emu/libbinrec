 ...
 *
 * If fold_fp is true, floating-point operations on constants will also be
 * folded.  Note that this can change the behavior of the translated code,
 * since constant folding does not attempt to match the precise behavior
 * of the guest architecture and any exceptions generated by the operation
 * are discarded.
 *
 * [Parameters]
 *     unit: RTL unit.
 *     fold_fp: True to fold floating-point operations, false to leave
 *         them unfolded.
 */
#define rtl_opt_fold_constants INTERNAL(rtl_opt_fold_constants)
extern void rtl_opt_fold_constants(RTLUnit *unit, bool fold_fp);
