...
/**
 * rtl_opt_fold_registers:  Perform various categories of register folding
 * on the given RTL unit.
 *
 * If fold_constants is true, convert instructions that operate on constant
 * operands into load-immediate instructions that load the result of the
 * operation, and convert register-register instructions in which one of
 * the two registers is constant into register-immediate instructions when
 * possible.
 *
 * If both fold_constants and fold_fp_constants are true, floating-point
 * operations on constants will also be folded.  Note that this can change
 * the behavior of the translated code, since constant folding does not
 * attempt to match the precise behavior of the guest architecture and any
 * exceptions generated by the operation are discarded.
 *
 * If fold_vectors is true, operations on scalar elements of vector
 * registers, such as VEXTRACT on a vector constructed with VBROADCAST,
 * will be converted to scalar operations on the original source values.
 *
 * [Parameters]
 *     unit: RTL unit.
 *     fold_constants: True to fold and propagate constants.
 *     fold_fp_constants: True to fold floating-point constant operations
 *         when fold_constants is true; false to leave them unfolded.
 *     fold_vectors: True to fold vector operations.
 */
#define rtl_opt_fold_registers INTERNAL(rtl_opt_fold_registers)
extern void rtl_opt_fold_registers(RTLUnit *unit, bool fold_constants,
 ...
 * floating-point type.
 */
static inline PURE_FUNCTION bool rtl_register_is_float(const RTLRegister *reg)
{
    return rtl_type_is_float(reg->type);
}
