 ...
 * with immediate offset, as seen in the example above) to be written as a
 * single instruction.  This is for convenience of translating operations
 * which are common across many architectures, since otherwise the effort
 * to break them down into multiple RTL instructions would be effectively
 * wasted when the output translator converted them back to the original
 * form.
 *
 ...
 * instruction is invalid, and will result in an assertion failure if
 * operand sanity checks are enabled.  Guest instructions which modify
 * (both read and write) an operand should create a second RTL register to
 * hold the result of the instruction.
 *
 ...
 * internally by an RTLInsn structure.
 */
...
typedef enum RTLOpcode {
    ...
    /* Note that for memory loads and stores, address operands must be of
     * ADDRESS type, offsets must be in [-32768,+32767], and the value
     * operand must match the load/store type (or INT32 tor 8/16-bit
     * integer loads). */
