...
int main(void)
{
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_VBUILD2, reg4, reg1, reg2, 0));
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_VBUILD2, reg5, reg3, reg1, 0));
    EXPECT_ICE("Operand constraint violated:"
               " unit->regs[src1].type == rtl_vector_element_type(unit->regs[dest].type)");
    EXPECT_EQ(unit->num_insns, 3);
    EXPECT(unit->error);
    unit->error = false;

    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_VBUILD2, reg5, reg1, reg3, 0));
    EXPECT_ICE("Operand constraint violated:"
