...
int main(void)
{
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_FCOPYROUND, reg3, reg1, reg1, 0));
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_FCOPYROUND, reg4, reg2, reg1, 0));
    EXPECT_ICE("Operand constraint violated:"
               " unit->regs[src1].type == RTLTYPE_FPSTATE");
    EXPECT_EQ(unit->num_insns, 2);
    EXPECT(unit->error);
    unit->error = false;

    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_FCOPYROUND, reg4, reg1, reg2, 0));
    EXPECT_ICE("Operand constraint violated:"
               " unit->regs[src2].type == RTLTYPE_FPSTATE");
    EXPECT_EQ(unit->num_insns, 2);
    EXPECT(unit->error);
    unit->error = false;
