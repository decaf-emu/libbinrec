...
int main(void)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_FTRUNCI, reg4, reg2, 0, 0));
    EXPECT(rtl_add_insn(unit, RTLOP_FTRUNCI, reg5, reg1, 0, 0));
    EXPECT(rtl_add_insn(unit, RTLOP_FTRUNCI, reg6, reg2, 0, 0));
    EXPECT_EQ(unit->num_insns, 6);
    EXPECT_EQ(unit->insns[2].opcode, RTLOP_FTRUNCI);
    EXPECT_EQ(unit->insns[2].dest, reg3);
    EXPECT_EQ(unit->insns[2].src1, reg1);
    EXPECT_EQ(unit->insns[3].opcode, RTLOP_FTRUNCI);
    EXPECT_EQ(unit->insns[3].dest, reg4);
    EXPECT_EQ(unit->insns[3].src1, reg2);
    EXPECT_EQ(unit->insns[4].opcode, RTLOP_FTRUNCI);
    EXPECT_EQ(unit->insns[4].dest, reg5);
    EXPECT_EQ(unit->insns[4].src1, reg1);
    EXPECT_EQ(unit->insns[5].opcode, RTLOP_FTRUNCI);
