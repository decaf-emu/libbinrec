...
int main(void)
{
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_SEQ, reg4, reg2, reg1, 0));
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_SEQ, reg4, reg2, reg3, 0));
    EXPECT_EQ(unit->num_insns, 4);
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_SEQ, reg5, reg2, reg3, 0));
    EXPECT_ICE("Operand constraint violated:"
