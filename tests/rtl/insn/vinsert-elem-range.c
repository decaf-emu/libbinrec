...
int main(void)
{
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_VINSERT, reg3, reg1, reg2, 2));
    EXPECT_ICE("Operand constraint violated:"
               " other < (unsigned int)rtl_vector_length(unit->regs[dest].type)");
    EXPECT_EQ(unit->num_insns, 2);
    EXPECT(unit->error);
    unit->error = false;

    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_VINSERT, reg3, reg1, reg2, -1));
    EXPECT_ICE("Operand constraint violated:"
               " other < (unsigned int)rtl_vector_length(unit->regs[dest].type)");
    EXPECT_EQ(unit->num_insns, 2);
    EXPECT(unit->error);
