...
int main(void)
{
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_FCMP, 0, reg1, reg2, RTLFCMP_LT));
    EXPECT_ICE("Operand constraint violated: dest != 0");
    EXPECT_EQ(unit->num_insns, 2);
    EXPECT(unit->error);
    unit->error = false;

    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_FCMP, reg3, 0, reg2, RTLFCMP_LT));
    EXPECT_ICE("Operand constraint violated: src1 != 0");
    EXPECT_EQ(unit->num_insns, 2);
    EXPECT(unit->error);
    unit->error = false;

    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_FCMP, reg3, reg1, 0, RTLFCMP_LT));
