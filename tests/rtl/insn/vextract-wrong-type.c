...
int main(void)
{
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_VEXTRACT, reg3, reg1, 0, 0));
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_VEXTRACT, reg4, reg2, 0, 1));
    EXPECT_ICE("Operand constraint violated:"
               " unit->regs[dest].type == rtl_vector_element_type(unit->regs[src1].type)");
    EXPECT_EQ(unit->num_insns, 2);
    EXPECT(unit->error);
    unit->error = false;
