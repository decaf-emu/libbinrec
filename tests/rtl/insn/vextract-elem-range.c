...
int main(void)
{
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_VEXTRACT, reg2, reg1, 0, 2));
    EXPECT_ICE("Operand constraint violated:"
               " other < (unsigned int)rtl_vector_length(unit->regs[src1].type)");
    EXPECT_EQ(unit->num_insns, 1);
    EXPECT(unit->error);
    unit->error = false;

    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_VEXTRACT, reg2, reg1, 0, -1));
    EXPECT_ICE("Operand constraint violated:"
               " other < (unsigned int)rtl_vector_length(unit->regs[src1].type)");
    EXPECT_EQ(unit->num_insns, 1);
    EXPECT(unit->error);
