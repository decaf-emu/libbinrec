...
int main(void)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_SEXT16, reg3, reg1, 0, 0));
    EXPECT(rtl_add_insn(unit, RTLOP_SEXT16, reg4, reg2, 0, 0));
    EXPECT_EQ(unit->num_insns, 4);
    EXPECT_EQ(unit->insns[2].opcode, RTLOP_SEXT16);
    EXPECT_EQ(unit->insns[2].dest, reg3);
    EXPECT_EQ(unit->insns[2].src1, reg1);
    EXPECT_EQ(unit->insns[3].opcode, RTLOP_SEXT16);
