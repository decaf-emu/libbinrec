...
int main(void)
{
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_CMPXCHG, 0, reg1, reg2, reg3));
    EXPECT_ICE("Operand constraint violated: dest != 0");
    EXPECT_EQ(unit->num_insns, 3);
    EXPECT(unit->error);
    unit->error = false;

    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_CMPXCHG, reg4, 0, reg2, reg3));
    EXPECT_ICE("Operand constraint violated: src1 != 0");
    EXPECT_EQ(unit->num_insns, 3);
    EXPECT(unit->error);
    unit->error = false;

    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_CMPXCHG, reg4, reg1, 0, reg3));
    EXPECT_ICE("Operand constraint violated: src2 != 0");
    EXPECT_EQ(unit->num_insns, 3);
    EXPECT(unit->error);
    unit->error = false;

    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_CMPXCHG, reg4, reg1, reg2, 0));
