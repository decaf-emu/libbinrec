...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_SGTS, reg5, reg1, reg2, 0));
    EXPECT(reg6 = rtl_alloc_register(unit, RTLTYPE_ADDRESS));
    EXPECT(rtl_add_insn(unit, RTLOP_SGTS, reg6, reg1, reg3, 0));
    EXPECT(reg7 = rtl_alloc_register(unit, RTLTYPE_ADDRESS));
    EXPECT(rtl_add_insn(unit, RTLOP_SGTS, reg7, reg1, reg4, 0));
...
static const char expected[] =
        ...
        "[info] Folded r7 to constant value 0x1 at 6\n"
