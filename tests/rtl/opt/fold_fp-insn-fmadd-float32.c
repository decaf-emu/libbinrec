...
static int add_rtl(RTLUnit *unit)
{
    ...
    /* Also verify that it's a true FMA and not separate operations.
     * The precise product reg5 * reg6 has binary 010 following the least
     * significant mantissa bit; we add 011 in the same bit position to
     * cause the final result to round up rather than down.  If the
     * operations are separate, the product will round down and the addend
     * will then not be large enough to cause the final result to round up.
     * (This holds true regardless of rounding mode, though if the runtime
     * environment defaults to round-toward-zero or round-down, a true FMA
     * will also fail here.) */
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_FMADD, reg8, reg5, reg6, reg7));
...
static const char expected[] =
        ...
        "[info] Folded r8 to constant value 0x3FA00002 at 7\n"
