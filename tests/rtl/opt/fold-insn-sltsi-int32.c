...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_SLTSI, reg2, reg1, 0, 1234));
    EXPECT(reg3 = rtl_alloc_register(unit, RTLTYPE_INT32));
    EXPECT(rtl_add_insn(unit, RTLOP_SLTSI, reg3, reg1, 0, 1));
    EXPECT(reg4 = rtl_alloc_register(unit, RTLTYPE_INT32));
    EXPECT(rtl_add_insn(unit, RTLOP_SLTSI, reg4, reg1, 0, -1));
    EXPECT(reg5 = rtl_alloc_register(unit, RTLTYPE_INT32));
    EXPECT(rtl_add_insn(unit, RTLOP_SLTSI, reg5, reg1, 0, 5678));
    EXPECT(reg6 = rtl_alloc_register(unit, RTLTYPE_INT32));
    EXPECT(rtl_add_insn(unit, RTLOP_SLTSI, reg6, reg1, 0, -5678));
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_SLTSI, reg8, reg7, 0, 1));
...
static const char expected[] =
        ...
        "[info] Folded r5 to constant value 1 at 4\n"
        "[info] Folded r6 to constant value 0 at 5\n"
        "[info] r1 no longer used, setting death = birth\n"
        "[info] Folded r8 to constant value 1 at 7\n"
