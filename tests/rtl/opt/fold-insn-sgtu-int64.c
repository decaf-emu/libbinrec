...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_SGTU, reg5, reg1, reg2, 0));
    EXPECT(reg6 = rtl_alloc_register(unit, RTLTYPE_ADDRESS));
    EXPECT(rtl_add_insn(unit, RTLOP_SGTU, reg6, reg1, reg3, 0));
    EXPECT(reg7 = rtl_alloc_register(unit, RTLTYPE_ADDRESS));
    EXPECT(rtl_add_insn(unit, RTLOP_SGTU, reg7, reg1, reg4, 0));
...
static const char expected[] =
        ...
        "[info] Folded r5 to constant value 0x1 at 4\n"
        "[info] r2 no longer used, setting death = birth\n"
        "[info] Folded r6 to constant value 0x0 at 5\n"
        "[info] r3 no longer used, setting death = birth\n"
        "[info] Folded r7 to constant value 0x0 at 6\n"
