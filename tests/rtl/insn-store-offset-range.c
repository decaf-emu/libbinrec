...
int main(void)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_STORE, 0, reg1, reg2, 0x7FFF));
    EXPECT(rtl_add_insn(unit, RTLOP_STORE, 0, reg1, reg2, -0x8000));
    EXPECT_EQ(unit->num_insns, 5);
    EXPECT_EQ(unit->insns[2].opcode, RTLOP_STORE);
    EXPECT_EQ(unit->insns[2].src1, reg1);
    EXPECT_EQ(unit->insns[2].src2, reg2);
    EXPECT_EQ(unit->insns[2].offset, 0);
    EXPECT_EQ(unit->insns[3].opcode, RTLOP_STORE);
    EXPECT_EQ(unit->insns[3].src1, reg1);
    EXPECT_EQ(unit->insns[3].src2, reg2);
    EXPECT_EQ(unit->insns[3].offset, 0x7FFF);
    EXPECT_EQ(unit->insns[4].opcode, RTLOP_STORE);
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_STORE, 0, reg1, reg2, 0x8000));
    EXPECT_ICE("Operand constraint violated:"
               " other <= 0x7FFF || other >= UINT64_C(-0x8000)");
    EXPECT_EQ(unit->num_insns, 5);
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_STORE, 0, reg1, reg2, -0x8001));
    EXPECT_ICE("Operand constraint violated:"
               " other <= 0x7FFF || other >= UINT64_C(-0x8000)");
    EXPECT_EQ(unit->num_insns, 5);
#endif
