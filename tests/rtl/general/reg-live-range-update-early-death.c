...
int main(void)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_ADDI, reg3, reg2, 0, 30));
    ...
    /* reg1's live range is not touched since it dies before the label. */
    EXPECT_EQ(unit->regs[reg1].birth, 0);
    EXPECT_EQ(unit->regs[reg1].death, 1);
    /* reg2's live range is extended to the backward branch. */
    EXPECT_EQ(unit->regs[reg2].birth, 1);
    EXPECT_EQ(unit->regs[reg2].death, 4);
    /* reg3 is not live on entering the second block, so its live range
     * is not extended. */
    EXPECT_EQ(unit->regs[reg3].birth, 3);
    EXPECT_EQ(unit->regs[reg3].death, 3);
