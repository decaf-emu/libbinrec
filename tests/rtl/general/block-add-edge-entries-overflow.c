...
int main(void)
{
    ...
    EXPECT_EQ(unit->blocks[8].next_block, -2);  // Unique negative value.
    EXPECT_EQ(unit->blocks[8].prev_block, -3);  // Unique negative value.
    ...
    EXPECT_EQ(unit->blocks[8].exits[0], -4);  // Unique negative value.
    EXPECT_EQ(unit->blocks[8].exits[1], -5);  // Unique negative value.
    EXPECT_FALSE(unit->error);
    EXPECT_STREQ(get_log_messages(), NULL);

    /* Adding another block should link to the last code block (7), not
     * the dummy block (8). */
    EXPECT(rtl_block_add(unit));
    EXPECT_EQ(unit->num_blocks, 10);
    EXPECT_EQ(unit->blocks[9].next_block, -1);
    EXPECT_EQ(unit->blocks[9].prev_block, 7);
    EXPECT_EQ(unit->blocks[7].next_block, 9);
