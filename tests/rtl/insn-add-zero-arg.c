...
int main(void)
{
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_ADD, 0, reg1, reg2, 0));
    EXPECT_ICE("Operand constraint violated: dest != 0");
    EXPECT_EQ(unit->num_insns, 2);
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_ADD, reg3, 0, reg2, 0));
    EXPECT_ICE("Operand constraint violated: src1 != 0");
    EXPECT_EQ(unit->num_insns, 2);
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_ADD, reg3, reg1, 0, 0));
