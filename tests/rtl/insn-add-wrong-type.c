...
int main(void)
{
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_ADD, reg4, reg2, reg1, 0));
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_ADD, reg4, reg1, reg3, 0));
    EXPECT_ICE("Operand constraint violated:"
    ...
    EXPECT_FALSE(rtl_add_insn(unit, RTLOP_ADD, reg4, reg2, reg3, 0));
    EXPECT_ICE("Operand constraint violated:"
               " unit->regs[src1].type == unit->regs[dest].type");
    EXPECT_EQ(unit->num_insns, 3);
    EXPECT(unit->error);
