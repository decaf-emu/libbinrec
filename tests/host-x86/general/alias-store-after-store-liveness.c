...
static int add_rtl(RTLUnit *unit)
{
    ...
    /* If reg2 is not properly killed, it will trigger an error at the
     * beginning of this block because it is dead but in the live set (or
     * the self-loop will trigger an assertion failure if operand sanity
     * checks are disabled). */
    int label, reg4;
