...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(copy = rtl_alloc_register(unit, RTLTYPE_INT32));
    EXPECT(rtl_add_insn(unit, RTLOP_MOVE, copy, spill, 0, 0));
    ...
    int last_sum = sum;
    EXPECT(sum = rtl_alloc_register(unit, RTLTYPE_INT32));
    EXPECT(rtl_add_insn(unit, RTLOP_ADD, sum, last_sum, copy, 0));
    EXPECT(rtl_add_insn(unit, RTLOP_NOP, 0, spill, 0, 0));
...
static const uint8_t expected_code[] = {
    ...
    0x41,0xBC,0x0A,0x00,0x00,0x00,      // mov $10,%r12d
    0x41,0xBD,0x0B,0x00,0x00,0x00,      // mov $11,%r13d
    0x41,0xBE,0x0C,0x00,0x00,0x00,      // mov $12,%r14d
    ...
    /* This jump will cross the spill of EAX, so a manual reload is needed. */
