...
static int add_rtl(RTLUnit *unit)
{
    ...
    /* reg5 should not get EAX since it's also a CMPXCHG output.
     * (reg2 can't get it in any case because it overlaps reg3.) */
    EXPECT(rtl_add_insn(unit, RTLOP_MULHU, reg6, reg2, reg5, 0));
    EXPECT(rtl_add_insn(unit, RTLOP_NOP, 0, reg2, reg5, 0));
