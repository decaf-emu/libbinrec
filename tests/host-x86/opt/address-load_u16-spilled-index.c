...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_ADD, reg3, reg2, reg1, 0));
    for (int i = 0; i < lenof(dummy_regs); i++) {
        EXPECT(rtl_add_insn(unit, RTLOP_NOP, 0, dummy_regs[i], 0, 0));
    }
    EXPECT(rtl_add_insn(unit, RTLOP_NOP, 0, reg1, 0, 0));
    EXPECT(reg4 = rtl_alloc_register(unit, RTLTYPE_INT32));
    EXPECT(rtl_add_insn(unit, RTLOP_LOAD_U16, reg4, reg3, 0, 0));
...
static const uint8_t expected_code[] = {
    ...
    0x45,0x0F,0xB7,0x34,0x06,           // movzwl (%r14,%rax),%r14d
...
static const char expected_log[] =
        ...
        "[info] Extending r15 live range to 30\n"
        "[info] Extending r14 live range to 30\n"
    #endif
    "";
