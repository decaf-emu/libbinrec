...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_FGETSTATE, reg4, 0, 0, 0));  // Gets EAX.
    EXPECT(reg5 = rtl_alloc_register(unit, RTLTYPE_FPSTATE));
    EXPECT(rtl_add_insn(unit, RTLOP_FGETSTATE, reg5, 0, 0, 0));  // Gets EDX.
    EXPECT(reg6 = rtl_alloc_register(unit, RTLTYPE_FPSTATE));
    /* The temporary for merging rounding fields should not clobber ECX. */
    EXPECT(rtl_add_insn(unit, RTLOP_FCOPYROUND, reg6, reg4, reg5, 0));
...
static const uint8_t expected_code[] = {
    ...
    0x81,0xE0,0xFF,0x9F,0x00,0x00,      // and $0x9FFF,%eax
    0x8B,0xF2,                          // mov %edx,%esi
    0x81,0xE6,0x00,0x60,0x00,0x00,      // and $0x6000,%esi
    0x0B,0xC6,                          // or %esi,%eax
