...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_CMPXCHG, reg5, reg1, reg2, reg3));
    EXPECT(reg6 = rtl_alloc_register(unit, RTLTYPE_INT32));
    /* reg4 should not get assigned to EAX since reg2 is already in EAX
     * and is still live here. */
    EXPECT(rtl_add_insn(unit, RTLOP_CMPXCHG, reg6, reg1, reg4, reg3));
