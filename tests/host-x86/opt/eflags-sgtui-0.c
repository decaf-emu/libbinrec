...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_ANDI, reg2, reg1, 0, 2));
    EXPECT(reg3 = rtl_alloc_register(unit, RTLTYPE_INT32));
    EXPECT(rtl_add_insn(unit, RTLOP_SGTUI, reg3, reg2, 0, 0));
...
static const uint8_t expected_code[] = {
    ...
    /* SGTUI(0) should accept test-only flag states, so there should be
