...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_MULI, reg3, reg1, 0, 2));
    EXPECT(reg4 = rtl_alloc_register(unit, RTLTYPE_INT32));
    EXPECT(rtl_add_insn(unit, RTLOP_SLTSI, reg4, reg1, 0, 0));
    EXPECT(reg5 = rtl_alloc_register(unit, RTLTYPE_INT32));
    EXPECT(rtl_add_insn(unit, RTLOP_MULI, reg5, reg3, 0, 2));
...
static const uint8_t expected_code[] = {
    ...
    0x6B,0xC8,0x02,                     // imul $2,%eax,%ecx
    0x33,0xD2,                          // xor %edx,%edx
    0x85,0xC0,                          // test %eax,%eax
    0x0F,0x9C,0xC2,                     // setl %dl
    0x6B,0xC9,0x02,                     // imul $2,%ecx,%ecx
