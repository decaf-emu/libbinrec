...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_SCAST, reg2, reg1, 0, 0));

    alloc_dummy_registers(unit, 3, RTLTYPE_INT32);

    EXPECT(reg3 = rtl_alloc_register(unit, RTLTYPE_ADDRESS));
    EXPECT(rtl_add_insn(unit, RTLOP_SCAST, reg3, reg2, 0, 0));
    EXPECT(reg4 = rtl_alloc_register(unit, RTLTYPE_INT32));
    EXPECT(rtl_add_insn(unit, RTLOP_SCAST, reg4, reg3, 0, 0));
...
static const uint8_t expected_code[] = {
    ...
    0x44,0x8B,0xC7,                     // mov %edi,%r8d
    0x4D,0x63,0xC0,                     // movslq %r8d,%r8
    0x41,0x8B,0xD8,                     // mov %r8d,%ebx
    0x5B,                               // pop %rbx
    0xC3,                               // ret
};
