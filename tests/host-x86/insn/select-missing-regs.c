...
static int add_rtl(RTLUnit *unit)
{
     ...
     * instruction with missing registers, so we add a LOAD_IMM (to set
     * the live range on the destination register properly) and modify the
    ...
    unit->insns[unit->num_insns-1].src1 = 0;
    unit->insns[unit->num_insns-1].src2 = 0;
    unit->insns[unit->num_insns-1].cond = 0;
