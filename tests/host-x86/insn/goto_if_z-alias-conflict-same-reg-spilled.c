...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_LOAD_IMM, reg3, 0, 0, 3));  // Spills EAX.
    for (int i = 0; i < lenof(dummy_regs); i++) {
        EXPECT(rtl_add_insn(unit, RTLOP_NOP, 0, dummy_regs[i], 0, 0));
    }
    EXPECT(rtl_add_insn(unit, RTLOP_NOP, 0, reg2, 0, 0));

    /* This needs to reload reg2 if the branch is taken, but the reload
     * should be conditional so as not to clobber reg3. */
    EXPECT(rtl_add_insn(unit, RTLOP_GOTO_IF_Z, 0, reg3, 0, label));

    /* Store to the alias via EAX so it's merged through EAX in the
     * following block. */
    int reg4;
...
static const uint8_t expected_code[] = {
    ...
    0x83,0xC0,0x04,                     // L0: add $4,%eax
    0x83,0xC0,0x06,                     // L1: add $6,%eax
    0x89,0x87,0x34,0x12,0x00,0x00,      // mov %eax,0x1234(%rdi)
