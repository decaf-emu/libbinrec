...
static int add_rtl(RTLUnit *unit)
{
    ...
    /* We need a separate register to spill reg3 since FMADD will avoid
     * src3 if it's not already spilled. */
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_FMADD, reg5, reg1, reg2, reg3));
    for (int i = 0; i < lenof(dummy_regs); i++) {
...
static const uint8_t expected_code[] = {
    ...
    0x66,0x44,0x0F,0x6E,0xF0,           // movd %eax,%xmm14
    0x45,0x0F,0x28,0xF4,                // movaps %xmm12,%xmm14
    0xF3,0x45,0x0F,0x59,0xF5,           // mulss %xmm13,%xmm14
