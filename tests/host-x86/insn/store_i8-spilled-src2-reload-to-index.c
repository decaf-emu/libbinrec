...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(reg4 = rtl_alloc_register(unit, RTLTYPE_ADDRESS)); // Optimized out.
    EXPECT(rtl_add_insn(unit, RTLOP_ADD, reg4, reg3, reg2, 0));
    EXPECT(rtl_add_insn(unit, RTLOP_STORE_I8, 0, reg4, reg1, 0));
    for (int i = 0; i < lenof(dummy_regs); i++) {
...
static const uint8_t expected_code[] = {
    ...
    0x43,0x88,0x04,0x3E,                // mov %al,(%r14,%r15)
...
static const char expected_log[] =
        ...
        "[info] Extending r16 live range to 17\n"
    #endif
    "";
