...
static int add_rtl(RTLUnit *unit)
{
    ...
    EXPECT(rtl_add_insn(unit, RTLOP_NOP, 0, dummy_regs[1], 0, 0));
    EXPECT(reg2 = rtl_alloc_register(unit, RTLTYPE_INT64));
    EXPECT(rtl_add_insn(unit, RTLOP_LOAD_IMM, reg2, 0, 0, 2));  // Gets CX.
...
static const uint8_t expected_code[] = {
    ...
    0xF0,0x49,0x0F,0xB1,0x0F,           // lock cmpxchg %rcx,(%r15)
