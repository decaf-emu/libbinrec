...
static void post_insn_callback(void *state_, uint32_t address)
{
    ...
    saved_state[save_counter].nia = state->nia;
...
int main(void)
{
    ...
    EXPECT_EQ(saved_state[0].nia, 0x1004);
    EXPECT_EQ(saved_state[0].r3, 1);
    EXPECT_EQ(saved_state[0].r4, 0);
    EXPECT_EQ(saved_state[1].address, 0x1004);
    EXPECT_EQ(saved_state[1].nia, 0x1008);
    EXPECT_EQ(saved_state[1].r3, 1);
    EXPECT_EQ(saved_state[1].r4, 10);
    EXPECT_EQ(saved_state[2].address, 0x1008);
    EXPECT_EQ(saved_state[2].nia, 0x100C);
    EXPECT_EQ(saved_state[2].r3, 2);
    EXPECT_EQ(saved_state[2].r4, 10);
    EXPECT_EQ(saved_state[3].address, 0x100C);
    EXPECT_EQ(saved_state[3].nia, 0xFFFFFFFC); // RETURN_ADDRESS from execute.c
